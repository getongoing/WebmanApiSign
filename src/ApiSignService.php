...
class ApiSignService
{
    ...
    public function check(array $data, string $key)
    {
        ...
        if($this->config['replay']){
            ...
            $noncestr = Cache::get('noncestr:'.$data[$this->config['fields']['noncestr']].($ip ?? ''));
            if ($noncestr) {
                throw new ApiSignException("请求失效", ApiSignException::REQUEST_INVALID);
            }else{
                //存储 至noncestr目录下
                Cache::set('noncestr:'.$data[$this->config['fields']['noncestr']].($ip ?? ''), $data[$this->config['fields']['noncestr']], $this->config['replaytime']);
            }
        }
    }
}
